...
from bot.handlers.keyboards import DIGEST_CHOICES, next_rarity_mode
...
async def handle_chat_feed(request: web.Request) -> web.Response:
    ...
    try:
        payload = await build_feed_payload(
            repo, chat_id, locale=locale, limit=limit, month=month
        )
    except ValueError as exc:
...
async def handle_avatar(request: web.Request) -> web.Response:
    ...
    if bot is None:
        raise web.HTTPNotFound(text="no photo")
    result = await load_avatar_bytes(
        bot, tg_id, file_id=target.photo_file_id if target else None
    )
    if result is None:
        raise web.HTTPNotFound(text="no photo")
    body, mime = result
