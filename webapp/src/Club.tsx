...
function ClubStats({
  ...
  for (const row of online.filter((m) => m.playing && m.title_name)) {
    live.set(row.title_name as string, (live.get(row.title_name as string) ?? 0) + 1);
  }
  ...
  const rares = [...feed]
    ...
    .slice(0, 5);
  const liveRows = [...live.entries()].sort((a, b) => b[1] - a[1]);
  return (
    <>
      <header className="page-head is-split">
        <h1>{t(locale, "stats")}</h1>
      ...
      {liveRows.length > 0 ? (
        <section className="stat-block is-soft">
          <p className="stat-block-title">{t(locale, "liveGames")}</p>
          {liveRows.map(([name, count]) => (
            <div key={name} className="stat-line">
              <span>{name}</span>
              <strong>
                {count} {t(locale, "hunters")}
              </strong>
            </div>
          ))}
        </section>
